...
# Базовый стоп-список общей токенизации. Им пользуется и BM25 второго места:
# там служебные слова уже придавлены редкостью (idf), а их удаление роняло
# балл ниже абсолютного порога (замер 20.09.2026: три потерянных вопроса
# отложенной половины независимого набора), поэтому список короткий.
STOP_TOKENS = {
    "about", "актуальный", "весь", "где", "данные", "для", "как", "какой",
    "когда", "мне", "можно", "надо", "нужно", "помоги", "почему", "проверь",
    "проект", "расскажи", "сделай", "сейчас", "список", "статус", "задача",
    "задачи", "оперативный", "текущий", "только", "файл", "хочу", "what",
    "where", "with", "you", "your",
}
# Расширенный стоп-список только для отбора первого места по заголовку и
# строке index: там служебное слово весит 1 по index и 4 по заголовку без
# всякой редкости, и один шумный токен решает, перевалит ли кандидат порог.
# Проверяются точной формой (без лемматизации), поэтому частые словоформы
# перечислены отдельно; ё уже приведена к е. Список намеренно консервативный:
# слова, которые бывают содержанием записи (нет, есть, без, после, list,
# status, check), сюда не входят, а глаголы с общей основой у тем корпуса
# (напомни против напоминаний) тем более.
SCORE_STOP_TOKENS = {
    # русские
    "актуальный", "будет", "был", "была", "были", "было", "весь", "все", "где",
    "давай", "данные", "делать", "для", "если", "еще", "задача", "задачи",
    "зачем", "здесь", "или", "как", "какая", "какие", "какой", "когда",
    "которая", "которые", "который", "кто", "куда", "мне", "можно", "мой",
    "моя", "надо", "него", "нее", "них", "нужен", "нужна", "нужно", "нужны",
    "оперативный", "откуда", "помоги", "помню", "помнишь", "потом", "почему",
    "при", "про", "проверь", "проект", "просто", "расскажи", "сам", "сама",
    "сделай", "сделать", "сейчас", "скажи", "снова", "список", "статус", "так",
    "такой", "там", "текущий", "того", "тоже", "только", "тут", "уже", "файл",
    "хочу", "чем", "через", "что", "чтобы", "эта", "эти", "это", "этого",
    "этой", "этом", "этот",
    # английские
    "about", "and", "are", "can", "does", "for", "from", "have", "how", "into",
    "not", "please", "should", "that", "the", "this", "was", "were", "what",
...
def _tokens(text: str, stop: frozenset[str] | set[str] | None = None) -> tuple[str, ...]:
    stop = STOP_TOKENS if stop is None else stop
    result = []
    for token in TOKEN_RE.findall(text.casefold().replace("ё", "е")):
        if token in stop or token in STOP_TOKENS:
            continue
